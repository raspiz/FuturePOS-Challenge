...
gameScore = 0
local bgLoad = 0
local gameOverTone = false
local warningTone = false
restart = false -- global var. used by scene_overlay.lua to restart game
...
local warningSound = audio.loadStream("warning.mp3")
...
function scene:overlayEnded( event )
  ...
  if(gameRunning == false and restart == false)then
  mute = false;
  audio.resume();
...
order to check for 
Initial game conditions and ui items are set
--]]
function StartGame()
		bar:toFront()
		score:toFront()
		...
		elseif (gameScore >= scoreToBeat) then
			...
			-- resumes regular music if player got warning sound and won the level
			gameOverTone = false
			warningTone = false
			audio.resume(sdtrk)
		...
		elseif (timeLeft <= 0) then	
			storyboard.showOverlay("scene_gameover")
		elseif (timeLeft == 130 and warningTone == false) then
			audio.pause(sdtrk)		
			audio.play(warningSound)
			warningTone = true
		elseif (timeLeft == 20 and gameOverTone == false) then
