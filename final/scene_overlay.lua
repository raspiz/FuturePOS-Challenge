---------------------------------------------------------------------------------
-- mmmPop options menu overlay
-- This is the menu that can be brought up during the game. It will pause the game session and hold its state.
-- Options include:
	-- a volume up and down with responsive buttons. Minimum 0 (muted) max 10 (100%)
	-- restart game. this will end the current session and return to the main menu
	-- quit game. this will exit the game and return to the OS
	-- resume game. this will close the overlay and return to the game
...
function scene:createScene( event )
  ...
  gameVolumeString = gameVolume*10
	...
	local closeBar = display.newRect( display.contentCenterX, display.contentCenterY + 120, 100, 20 )
	closeBar:setFillColor(.1, .6, .8)
	...
	local restartBar = display.newRect( display.contentCenterX, display.contentCenterY + 20, 100, 20 )
	restartBar:setFillColor(.1, .6, .8)
	
	local restartButton = display.newText("Restart", 0,0, globals.font.regular, 18)
	restartButton.x = display.contentCenterX
	restartButton.y = display.contentCenterY + 20
	
	local quitBar = display.newRect( display.contentCenterX, display.contentCenterY + 70, 100, 20 )
	quitBar:setFillColor(.1, .6, .8)
	
	local quitButton = display.newText("Quit", 0, 0, globals.font.regular, 18)
...
local function QuitGame(event)
  ...
  group:insert(closeBar)
  group:insert(restartBar)
